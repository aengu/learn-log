"""Render DB → 로컬 Docker DB로 데이터 가져오기"""
import os
import subprocess
import tempfile
...
class Command(BaseCommand):
    help = "Render 프로덕션 DB를 로컬 Docker DB로 가져옵니다"
    ...
    def handle(self, *args, **options):
        ...
        if not options["no_confirm"]:
            self.stdout.write(
                self.style.WARNING(
                    "⚠ 로컬 DB의 모든 데이터가 Render DB 데이터로 교체됩니다."
                )
            )
        ...
        try:
            self._dump_remote(remote_url, dump_path)
            self._restore_local(local, dump_path)
        finally:
            if os.path.exists(dump_path):
                os.unlink(dump_path)

        self.stdout.write(self.style.SUCCESS("✓ dbpull 완료! Render → 로컬 동기화 성공"))
    ...
    def _dump_remote(self, url, dump_path):
        """Render DB를 pg_dump로 덤프"""
        self.stdout.write("Render DB 덤프 중...")
    ...
    def _restore_local(self, local, dump_path):
        """로컬 Docker DB에 복원"""
        self.stdout.write("로컬 DB 복원 중...")
        env = os.environ.copy()
        ...
        # pg_restore는 --clean으로 인해 warning이 나올 수 있어서 stderr 체크만
